...
def train_step(params, opt_state, xb, yb, lr, beta_one, beta_two, eps, step):
    ...
    # Gradient at logits via separate backward call
    dlogits = softmax_cross_entropy_backward(logits, yb)

    # Full backprop through network
    ...
    for layer in params:
        ...
        for pname in ['W', 'b']:
            ...
            grad_key = 'dW' if pname == 'W' else 'db'
...
def train_loop(params, x, y,
    ...
    # Build Adam state (zeros for m and v)
    opt_state = {}
    for layer in params:
        opt_state[layer] = {}
        for pname in ['W', 'b']:
    ...
    for epoch in range(num_epochs):
        params, opt_state, step_counter, losses = train_one_epoch(
            ...
            eps,
            step_counter,
            seed=seed + epoch   # different shuffle per epoch
        )
